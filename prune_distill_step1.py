...
############################################get initial grad for importance estimation############################################
        ...
        for i in tqdm(range(len(grad_iter)), desc="get initial grad for importance estimation"):
        ...
        print("===========================Embedding Pruning End===========================")
        
        #Bottleneck Aligning
        accum_steps = 4
        update_steps_per_epoch = math.ceil(len(train_loader) / accum_steps)
        total_updates = num_train_epochs * update_steps_per_epoch
        pbar = tqdm(total=total_updates, desc="Bottleneck Aligning", unit="step")
        ...
        for epoch in range(num_train_epochs):
            ...
            for i in range(len(train_iter)):
                ...
                if epoch<10:
                    loss = (1-a_weight)*loss_fn(student_embedding, teacher_embedding)+a_weight*loss_fn(student_qkv_emb1, teacher_qkv_emb1)+a_weight*loss_fn(student_qkv_emb2, teacher_qkv_emb2)+a_weight*loss_fn(student_mid_emb, teacher_mid_emb)
                    loss.backward()
                else:
